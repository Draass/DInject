...
namespace DInject.Asteroids
{
    public partial class GameInstaller : MonoInstaller
    {
        ...
        void InstallEvents()
        {
            // The original Zenject sample used the Signals subsystem here (SignalBusInstaller +
            // DeclareSignal<ShipCrashedSignal>). This DInject sample omits that optional subsystem and
            // instead binds a plain GameEvents hub - the ship-crashed flow works exactly the same way,
            // just via a C# event instead of a signal. See GameEvents / ShipStateDead / GameController.
